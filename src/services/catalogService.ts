...
export const saveCatalog = async (catalog: Partial<Catalog> & { title: string }): Promise<Catalog | null> => {
  // We don't include slug in the payload as it will be generated by the database trigger
  const { data, error } = await supabase
    .from('catalogs')
    .upsert({
      ...catalog
      // The slug field will be automatically generated by the database trigger
    })
    .select()
    .single();
