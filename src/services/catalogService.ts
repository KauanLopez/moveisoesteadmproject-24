...
import { Catalog, CatalogCategory, CatalogItem } from "@/types/catalogTypes";
...
export const fetchCatalogs = async (): Promise<(Catalog & { category_name?: string })[]> => {
  const { data, error } = await supabase
    .from('catalogs')
    .select(`
      *,
...
export const fetchCatalogBySlug = async (slug: string): Promise<(Catalog & { category_name?: string }) | null> => {
  const { data, error } = await supabase
    .from('catalogs')
    .select(`
      *,
...
export const saveCatalog = async (catalog: Partial<Catalog>): Promise<Catalog | null> => {
  const { data, error } = await supabase
    .from('catalogs')
    .upsert(catalog)
    .select()
...
export const saveCatalogItem = async (item: Partial<CatalogItem>): Promise<CatalogItem | null> => {
  const { data, error } = await supabase
    .from('catalog_items')
    .upsert(item)
    .select()
