...
const CatalogSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadCatalogs();
    // Adiciona um listener para atualizar se o localStorage mudar (útil para admin)
    window.addEventListener('storage', loadCatalogs);
    window.addEventListener('localStorageUpdated', loadCatalogs);

    return () => {
      window.removeEventListener('storage', loadCatalogs);
      window.removeEventListener('localStorageUpdated', loadCatalogs);
    };
  }, []);
